...
class ZoomMonitor():
    ...
    def sign_in(self, url):
        # The links that we receive prompt you to open the Zoom app if it's available. Replace the
        # domain name to skip that.
        ...
        self.driver.find_element(By.ID, "input-for-name").send_keys("Hand Raiser Bot")
        self.driver.find_element(By.CSS_SELECTOR, ".zm-btn").click()

        WebDriverWait(self.driver, 5).until(lambda _:
            len(self.driver.find_elements(By.CLASS_NAME, "SvgParticipantsDefault")) != 0)
        time.sleep(1) # The DOM isn't all set up yet; wait a little longer

        # We want to click on an item in the class "SvgParticipantsDefault" to open the
        # participants list. However, that element is not clickable, and instead throws an
        # exception that the click would be intercepted by its parent element, a div in the class
        # "footer-button-base__img-layer". So, instead let's look for all of those divs, and then
        # find the one that contains the participants image.
        ...
        # Now that we've clicked on the participants list, wait until it shows up.
        WebDriverWait(self.driver, 5).until(
    ...
    def count_hands(self):
        ...
        # the Unicode code point for the "hand raised" emoji). Elements whose class contains "270b"
        # show up in several places, however, so we restrict it to only the ones that are within
